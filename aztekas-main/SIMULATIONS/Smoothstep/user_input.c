   ...
   while(fscanf(paramfile, " %c", &t_firstChar) == 1 )
   {
      ...
      if(t_firstChar != '/' && t_firstChar != '%')
      {
         ...
         if(fscanf (paramfile, "%s = %s", t_key, t_value) == 2)
         {
            ...
            if(strcmp(t_key,"deltarho")==0)
            {
               deltarho = atof(t_value);
            }
         
            if(strcmp(t_key,"densamp")==0)
            {
               densamp = atof(t_value);
            }
            if(strcmp(t_key,"polarrho")==0)
            {
               polarrho = atof(t_value);
            }
            if(strcmp(t_key,"tr_width")==0)
            {
               tr_width = atof(t_value);
            }
         }
      }
